            ...
            {field.hasOther && (
              <label className="fill-option">
                <input
                  type="checkbox"
                  checked={!!(value?.other)}
                  onChange={(e) => onChange(field.id, { ...(value || {}), other: e.target.checked ? (value?.otherText || "") : undefined })}
                />
                ...
                {value?.other !== undefined && (
                  <input
                    ...
                    onChange={(e) => onChange(field.id, { ...(value || {}), otherText: e.target.value })}
                  />
                )}
              </label>
            )}
...
export default function FillFormPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (submissionId) {
          const res2 = await fetch(`/api/submissions/${submissionId}`, { headers: authHeaders() });
          if (res2.ok) {
  ...
  async function handleSubmit(isDraft) {
    setSubmitting(true);
    try {
      const body = { formId: parseInt(id), answers, isDraft };
      let res;
      ...
      if (!isDraft) {
        setTimeout(() => navigate("/inicio/preenchimentos"), 1500);
      }
    } catch (err) {
      showToast(err.message, true);
    } finally {
  ...
  return (
    <div className="fill-page">
      ...
      {/* Topbar */}
      <header className="fill-topbar">
        <div className="fill-topbar-left">
          <button className="fill-back-btn" onClick={() => navigate("/inicio")}>
          ...
          <h1 className="fill-topbar-title">{form.name}</h1>
          {isDraftMode && <span className="fill-draft-badge">Rascunho</span>}
        </div>
      </header>
      ...
      <main className="fill-content">
        <div className="fill-container">
          ...
          <div className="fill-intro">
            <h1>{form.name}</h1>
            <p>Preenche todos os campos e submete o formulário. Podes guardar um rascunho para continuar mais tarde.</p>
          </div>

          {/* Campos */}
          <div className="fill-card">
            {fields.length === 0 ? (
              <p style={{ color: "#9ca3af", fontSize: "14px" }}>Este formulário não tem campos configurados.</p>
            ) : (
              fields.map((field) => (
                <FieldInput
                  key={field.id}
